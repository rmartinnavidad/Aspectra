...
public:
    explicit LayerPoof(QWidget *parent,const QPoint &center):QWidget(parent){setAttribute(Qt::WA_TransparentForMouseEvents);setAttribute(Qt::WA_TranslucentBackground);resize(96,96);move(center-QPoint(48,48));raise();show();auto *anim=new QVariantAnimation(this);anim->setDuration(430);anim->setStartValue(0.0);anim->setEndValue(1.0);anim->setEasingCurve(QEasingCurve::OutCubic);QObject::connect(anim,&QVariantAnimation::valueChanged,this,[this](const QVariant &v){progress=v.toReal();update();});QObject::connect(anim,&QVariantAnimation::finished,this,&QObject::deleteLater);anim->start(QAbstractAnimation::DeleteWhenStopped);}
protected:
    void paintEvent(QPaintEvent *) override {QPainter p(this);p.setRenderHint(QPainter::Antialiasing);const QPointF c(width()*.5,height()*.5);const qreal t=progress;for(int i=0;i<12;++i){const qreal a=(6.28318530718*i/12.0)+(i%2?.12:-.08),d=8.0+34.0*t+(i%3)*3.0,r=qMax<qreal>(1.2,6.0*(1.0-t));QColor color=i%3==0?QColor("#7a4dff"):(i%3==1?QColor("#3ddcff"):QColor("#ffffff"));color.setAlphaF(qBound(0.0,1.0-t,1.0));p.setPen(Qt::NoPen);p.setBrush(color);p.drawEllipse(c+QPointF(std::cos(a)*d,std::sin(a)*d),r,r);}QColor ring("#a98cff");ring.setAlphaF(qBound(0.0,.8*(1.0-t),1.0));p.setPen(QPen(ring,2.0));p.setBrush(Qt::NoBrush);p.drawEllipse(c,10.0+26.0*t,10.0+26.0*t);}
private: qreal progress=0.0;
};
