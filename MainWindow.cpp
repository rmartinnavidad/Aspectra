...
public:
    explicit LayerPoof(QWidget *parent,const QPixmap &card,const QRect &from,const QRect &to):QWidget(parent),card(card),from(from),to(to){setAttribute(Qt::WA_TransparentForMouseEvents);setAttribute(Qt::WA_TranslucentBackground);setGeometry(parent->rect());raise();show();auto *anim=new QVariantAnimation(this);anim->setDuration(720);anim->setStartValue(0.0);anim->setEndValue(1.0);anim->setEasingCurve(QEasingCurve::Linear);QObject::connect(anim,&QVariantAnimation::valueChanged,this,[this](const QVariant &v){progress=v.toReal();update();});QObject::connect(anim,&QVariantAnimation::finished,this,&QObject::deleteLater);anim->start(QAbstractAnimation::DeleteWhenStopped);}
protected:
    void paintEvent(QPaintEvent *) override {QPainter p(this);p.setRenderHint(QPainter::Antialiasing);p.setRenderHint(QPainter::SmoothPixmapTransform);const qreal t=progress,fly=qMin<qreal>(1.0,t/.72),e=1-std::pow(1-fly,3),pop=qBound<qreal>(0.0,(t-.60)/.40,1.0);const QPointF a=from.center(),b=to.center(),arc(0,-qMin<qreal>(54.0,qAbs(b.x()-a.x())*.12+24.0));const QPointF center=a*(1-e)+b*e+arc*(4*e*(1-e));const qreal squash=1.0-.90*e,cardW=qMax<qreal>(8.0,from.width()*squash),cardH=qMax<qreal>(6.0,from.height()*(1.0-.78*e));QRectF ghost(center.x()-cardW*.5,center.y()-cardH*.5,cardW,cardH);if(fly<1.0&&!card.isNull()){for(int i=3;i>=1;--i){const qreal lag=qMax<qreal>(0.0,e-i*.055);const QPointF trail=a*(1-lag)+b*lag+arc*(4*lag*(1-lag));QRectF r(trail.x()-cardW*.5,trail.y()-cardH*.5,cardW,cardH);p.setOpacity(.07*i*(1-e));p.drawPixmap(r,card,card.rect());}p.setOpacity(qBound<qreal>(0.0,1.0-e*.62,1.0));p.drawPixmap(ghost,card,card.rect());}p.setOpacity(1.0);const QPointF c=b;for(int i=0;i<22;++i){const qreal angle=6.28318530718*i/22.0+(i%2?.11:-.07),distance=(10.0+(i%5)*3.0)+52.0*pop,radius=qMax<qreal>(1.0,(7.0-(i%3))*std::pow(1-pop,.7));QColor color=i%3==0?QColor("#7a4dff"):(i%3==1?QColor("#3ddcff"):QColor("#ffffff"));color.setAlphaF(qBound<qreal>(0.0,1.0-pop,1.0));p.setPen(Qt::NoPen);p.setBrush(color);p.drawEllipse(c+QPointF(std::cos(angle)*distance,std::sin(angle)*distance),radius,radius);}if(pop>0){QColor ring("#a98cff");ring.setAlphaF(qBound<qreal>(0.0,.95*(1-pop),1.0));p.setPen(QPen(ring,3.0*(1-pop)+1.0));p.setBrush(Qt::NoBrush);p.drawEllipse(c,8.0+46.0*pop,8.0+46.0*pop);QColor core("#ffffff");core.setAlphaF(qBound<qreal>(0.0,.75*(1-pop),1.0));p.setPen(Qt::NoPen);p.setBrush(core);p.drawEllipse(c,10.0*(1-pop),10.0*(1-pop));}}
private: QPixmap card;QRect from,to;qreal progress=0.0;
};
