...
void MainWindow::openProjectFile(const QString &path){
    if(galleryScreen)galleryScreen->hide();
    QFile in(path);if(!in.open(QIODevice::ReadOnly)){showError("Could not open project.");return;}const QJsonDocument document=QJsonDocument::fromJson(in.readAll());if(!document.isObject()){showError("This is not a valid Aspectra project.");return;}const QJsonObject root=document.object(),canvas=root.value("canvas").toObject();const int width=canvas.value("width").toInt(1024),height=canvas.value("height").toInt(1024);QStringList sources;canvasLayers.clear();for(const auto &value:root.value("layers").toArray()){const QJsonObject item=value.toObject();const QString source=item.value("source").toString();if(source.isEmpty()&& !source.startsWith(QLatin1String("aspectra://")))continue;if(!source.startsWith(QLatin1String("aspectra://"))&&!QFileInfo::exists(source))continue;sources<<source;CanvasLayer layer;layer.source=source;layer.name=item.value("name").toString(QFileInfo(source).completeBaseName());layer.position={item.value("x").toDouble(),item.value("y").toDouble()};layer.nativeSize={item.value("width").toInt(),item.value("height").toInt()};layer.visible=item.value("visible").toBool(true);canvasLayers[source]=layer;}setDimensions(width,height);if(sources.isEmpty()){createNewProject(width,height,QSettings().value("newProjectResolution",300).toInt(),true,QSettings().value("newProjectColorMode","RGB").toString(),QSettings().value("newProjectProfile","sRGB IEC61966-2.1").toString(),Qt::transparent);status->setText("Recovered blank canvas");}else{batch.files.clear();for(const QString &source:sources){if(source.startsWith(QLatin1String("aspectra://")))batch.files.append(source);else batch.add({source});}updateBatchLabel();selectFile(batch.files.first());status->setText(QString("Project opened · %1 linked artboards").arg(batch.files.size()));}if(!path.endsWith("Aspectra-X-recovery.aspectra"))rememberRecentDocument(path);
}
#if 0
