...
QImage ImageProcessor::compositeTimeline(const QImage &source,const QVector<TimelineTrack> &tracks,double time){
    ...
    for(const auto &track:tracks){
        ...
        if(!track.mask.isNull()){QImage alpha=track.mask.scaled(layer.size(),Qt::IgnoreAspectRatio,Qt::SmoothTransformation).convertToFormat(QImage::Format_Grayscale8);layer=layer.convertToFormat(QImage::Format_ARGB32);for(int y=0;y<layer.height();++y){QRgb *pixels=reinterpret_cast<QRgb*>(layer.scanLine(y));const uchar *mask=alpha.constScanLine(y);for(int x=0;x<layer.width();++x)pixels[x]=qRgba(qRed(pixels[x]),qGreen(pixels[x]),qBlue(pixels[x]),qAlpha(pixels[x])*mask[x]/255);}}
