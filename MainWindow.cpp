...
void MainWindow::buildUi(){
    ...
    auto *settingCard=new QWidget(settingsHost);settingCard->setObjectName("ActiveSettingCard");auto *settingCardLayout=new QHBoxLayout(settingCard);settingCardLayout->setContentsMargins(12,6,12,8);settingCardLayout->setSpacing(10);settingsLayout->insertWidget(2,settingCard);settingCard->hide();connect(subButtons,&QButtonGroup::idClicked,this,[this,settingsHost,settingCard,settingCardLayout,subNames](int index){QTimer::singleShot(0,this,[this,settingsHost,settingCard,settingCardLayout,subNames,index]{while(auto *item=settingCardLayout->takeAt(0)){delete item->widget();delete item;}const QString name=subNames.value(tabs->currentIndex()).value(index,"Setting");auto *icon=new QToolButton;icon->setIcon(settingIcon(name));icon->setIconSize(QSize(22,22));icon->setToolButtonStyle(Qt::ToolButtonIconOnly);icon->setToolTip(name);icon->setEnabled(false);icon->setFixedSize(30,30);settingCardLayout->addWidget(icon);auto *title=label(name);title->setFixedWidth(76);settingCardLayout->addWidget(title);QSlider *target=qobject_cast<QSlider*>(QApplication::focusWidget());if(!target||!tabs->isAncestorOf(target)){const auto controls=tabs->currentWidget()->findChildren<QSlider*>();target=controls.value(index%qMax(1,controls.size()),nullptr);}if(target){auto *slider=new GradientSlider;slider->setRange(target->minimum(),target->maximum());slider->setValue(target->value());slider->setToolTip(target->toolTip());slider->setObjectName("ActiveSettingSlider");settingCardLayout->addWidget(slider,1);connect(slider,&QSlider::valueChanged,target,&QSlider::setValue);connect(target,&QSlider::valueChanged,slider,[slider](int v){QSignalBlocker block(slider);slider->setValue(v);});}else settingCardLayout->addWidget(label("This setting opens its action in the active category.","muted"),1);settingCard->show();tabs->hide();settingsHost->setMinimumHeight(qMin(320,settingsHost->layout()->sizeHint().height()));settingsHost->updateGeometry();if(auto *outer=qobject_cast<QVBoxLayout*>(settingsHost->parentWidget()->layout())){outer->invalidate();outer->activate();}if(auto *active=dynamic_cast<GradientSlider*>(settingCard->findChild<QSlider*>("ActiveSettingSlider")))active->setFocus(Qt::OtherFocusReason);});});connect(tabs,&QTabWidget::currentChanged,this,[subButtons](int){QTimer::singleShot(0,subButtons,[subButtons]{if(auto *first=subButtons->button(0))first->click();});});
