...
class ValueDial final : public QWidget {
public:
    explicit ValueDial(QWidget *parent=nullptr):QWidget(parent){setFixedSize(38,38);hide();}
    void setRange(int low,int high){minimum=low;maximum=qMax(low+1,high);update();}
    void setValue(int next){value=next;update();}
protected:
    void paintEvent(QPaintEvent *) override {QPainter painter(this);painter.setRenderHint(QPainter::Antialiasing);const QRectF circle=rect().adjusted(2,2,-2,-2);const double amount=qBound(0.,double(value-minimum)/double(maximum-minimum),1.);painter.setPen(QPen(Qt::white,1.5));painter.setBrush(Qt::black);painter.drawEllipse(circle);painter.setPen(Qt::NoPen);painter.setBrush(QColor("#7a4dff"));painter.drawPie(circle,90*16,-qRound(amount*5760));painter.setPen(Qt::white);painter.setFont(QFont("Segoe UI",9,QFont::DemiBold));painter.drawText(circle,Qt::AlignCenter,QString::number(value));}
private:
...
void MainWindow::buildUi(){
    ...
    auto *stageHost=new QWidget(shell);stageHost->setObjectName("StageHost");stageHost->setMinimumHeight(240);stageHost->setMaximumHeight(qMax(240,qRound(height()*.43)));auto *stageColumn=new QVBoxLayout(stageHost);stageColumn->setContentsMargins(0,0,0,0);stageColumn->setSpacing(0);layout->addWidget(stageHost,0);
    stageHost->setMaximumHeight(QWIDGETSIZE_MAX);preview=new PreviewWidget;preview->setObjectName("creativeStage");auto *mainPreview=new MainPreview(preview,stageHost);auto *previewLayer=new QWidget(stageHost);auto *previewStack=new QStackedLayout(previewLayer);previewStack->setContentsMargins(0,0,0,0);previewStack->setStackingMode(QStackedLayout::StackAll);previewStack->addWidget(mainPreview);auto *safeZone=new QSlider(Qt::Vertical,previewLayer);safeZone->setObjectName("SafeZoneSlider");safeZone->setRange(0,45);safeZone->setValue(10);safeZone->setFixedWidth(32);safeZone->setSizePolicy(QSizePolicy::Fixed,QSizePolicy::Expanding);safeZone->setToolTip("Safe-zone margin. Drag to apply the same margin to every edge live.");previewStack->addWidget(safeZone);previewStack->setAlignment(safeZone,Qt::AlignLeft);stageColumn->addWidget(previewLayer,1);connect(safeZone,&QSlider::valueChanged,this,[this](int value){for(auto *control:safeInputs)control->setValue(value);refresh();});
    ...
    mainPreview->setMinimumHeight(360);mainPreview->setStyleSheet("background:transparent;border:none;");preview->setStyleSheet("background:transparent;border:none;");
    ...
    layout->removeWidget(stageHost);layout->removeWidget(carouselScroll);layout->removeWidget(tabs);auto *settingsHost=new QWidget(shell);settingsHost->setObjectName("SettingsHost");auto *settingsLayout=new QVBoxLayout(settingsHost);settingsLayout->setContentsMargins(0,0,0,0);settingsLayout->setSpacing(0);settingsLayout->addWidget(carouselScroll);settingsLayout->addWidget(tabs,1);auto *workspaceSplit=new QSplitter(Qt::Vertical,shell);workspaceSplit->setObjectName("PreviewSettingsSplit");workspaceSplit->setHandleWidth(0);workspaceSplit->setChildrenCollapsible(false);stageHost->setMinimumHeight(0);stageHost->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Ignored);mainPreview->setMinimumHeight(0);preview->setMinimumHeight(0);settingsHost->setMinimumHeight(0);settingsHost->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Ignored);workspaceSplit->setMinimumHeight(0);workspaceSplit->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Ignored);workspaceSplit->addWidget(stageHost);workspaceSplit->addWidget(settingsHost);workspaceSplit->handle(1)->setEnabled(false);workspaceSplit->handle(1)->hide();layout->addWidget(workspaceSplit,1);tabs->hide();settingsHost->setFixedHeight(110);workspaceSplit->setSizes({qMax(160,height()-110),110});auto resizeVideoStage=[this,workspaceSplit]{if(VideoProcessor::isVideo(currentFile)){const int controls=videoRow->sizeHint().height();const int previewHeight=qBound(150,workspaceSplit->sizes().value(0)-controls-8,360);preview->setFixedHeight(previewHeight);}};connect(workspaceSplit,&QSplitter::splitterMoved,this,[resizeVideoStage](int,int){resizeVideoStage();});auto settingsExpanded=std::make_shared<bool>(false);auto showSettings=[this,settingsHost,workspaceSplit,previewZoom,settingsExpanded,resizeVideoStage]{preview->setViewZoom(1.0);previewZoom->setValue(100);if(!*settingsExpanded){*settingsExpanded=true;tabs->show();constexpr int expandedRailStackHeight=320;settingsHost->setFixedHeight(expandedRailStackHeight);const int space=workspaceSplit->height();workspaceSplit->setSizes({qMax(160,space-expandedRailStackHeight),expandedRailStackHeight});QTimer::singleShot(0,this,resizeVideoStage);}};connect(tabButtons,&QButtonGroup::idClicked,this,[this,showSettings](int index){tabs->setCurrentIndex(index);showSettings();});
    ...
    auto *settingCard=new QWidget(settingsHost);settingCard->setObjectName("ActiveSettingCard");auto *settingCardLayout=new QHBoxLayout(settingCard);settingCardLayout->setContentsMargins(12,6,12,8);settingCardLayout->setSpacing(10);settingsLayout->insertWidget(2,settingCard);settingCard->hide();connect(subButtons,&QButtonGroup::idClicked,this,[this,settingCard,settingCardLayout,subNames](int index){QTimer::singleShot(0,this,[this,settingCard,settingCardLayout,subNames,index]{while(auto *item=settingCardLayout->takeAt(0)){delete item->widget();delete item;}const QString name=subNames.value(tabs->currentIndex()).value(index,"Setting");auto *icon=new QToolButton;icon->setIcon(settingIcon(name));icon->setIconSize(QSize(22,22));icon->setToolButtonStyle(Qt::ToolButtonIconOnly);icon->setToolTip(name);icon->setEnabled(false);icon->setFixedSize(30,30);settingCardLayout->addWidget(icon);auto *title=label(name);title->setFixedWidth(76);settingCardLayout->addWidget(title);auto *target=qobject_cast<QSlider*>(QApplication::focusWidget());if(target){auto *slider=new GradientSlider;slider->setRange(target->minimum(),target->maximum());slider->setValue(target->value());slider->setToolTip(target->toolTip());auto *value=new ValueDial;value->setRange(target->minimum(),target->maximum());value->setValue(target->value());value->setToolTip("Live value: "+QString::number(target->value()));settingCardLayout->addWidget(slider,1);settingCardLayout->addWidget(value);connect(slider,&QSlider::valueChanged,target,&QSlider::setValue);connect(target,&QSlider::valueChanged,slider,[slider](int v){QSignalBlocker block(slider);slider->setValue(v);});connect(target,&QSlider::valueChanged,value,[value](int v){value->setValue(v);value->setToolTip("Live value: "+QString::number(v));});}else settingCardLayout->addWidget(label("This setting opens its action in the active category.","muted"),1);settingCard->show();tabs->hide();});});connect(tabs,&QTabWidget::currentChanged,this,[subButtons](int){QTimer::singleShot(0,subButtons,[subButtons]{if(auto *first=subButtons->button(0))first->click();});});
    connect(subButtons,&QButtonGroup::idClicked,this,[this,settingCard,settingCardLayout,subNames](int index){if(!tabs||tabs->currentIndex()!=6||subNames.value(6).value(index)!="Sphere")return;QTimer::singleShot(1,this,[this,settingCard,settingCardLayout]{while(auto *item=settingCardLayout->takeAt(0)){delete item->widget();delete item;}auto *stack=new QVBoxLayout;stack->setContentsMargins(0,0,0,0);stack->setSpacing(5);auto addSphereLight=[stack](const QString &name,QSlider *target){if(!target)return;auto *line=new QHBoxLayout;line->setContentsMargins(0,0,0,0);auto *caption=label(name,"muted");caption->setFixedWidth(68);auto *control=new GradientSlider;control->setRange(target->minimum(),target->maximum());control->setValue(target->value());control->setToolTip(target->toolTip());line->addWidget(caption);line->addWidget(control,1);connect(control,&QSlider::valueChanged,target,&QSlider::setValue);connect(target,&QSlider::valueChanged,control,[control](int value){QSignalBlocker block(control);control->setValue(value);});stack->addLayout(line);};addSphereLight("AZIMUTH",lightAzimuth);addSphereLight("ELEVATION",lightElevation);addSphereLight("INTENSITY",lightIntensity);settingCardLayout->addLayout(stack,1);settingCard->show();});});
    connect(subButtons,&QButtonGroup::idClicked,this,[this,subNames,settingCardLayout](int index){if(!tabs||tabs->currentIndex()!=6)return;const QString name=subNames.value(6).value(index);if(auto *control=textureMapSliders.value(name,nullptr)){clearLayoutItems(settingCardLayout);control->setFocus(Qt::TabFocusReason);control->ensurePolished();if(textureMapPreview)textureMapPreview->setCurrentText(name);textureRefreshTimer.start();}});
    ...
    subRail->hide();connect(tabButtons,&QButtonGroup::idClicked,this,[subRail,subButtons]{subRail->show();QTimer::singleShot(0,subButtons,[subButtons]{if(auto *first=subButtons->button(0))first->click();});});auto collapseSettings=[this,settingsHost,workspaceSplit,settingsExpanded,subRail,settingCard]{if(!*settingsExpanded)return;*settingsExpanded=false;tabs->hide();subRail->hide();settingCard->hide();constexpr int collapsedRailStackHeight=110;settingsHost->setFixedHeight(collapsedRailStackHeight);workspaceSplit->setSizes({qMax(240,workspaceSplit->height()-collapsedRailStackHeight),collapsedRailStackHeight});if(VideoProcessor::isVideo(currentFile))preview->setFixedHeight(qBound(190,height()-640,360));if(!preview->parentWidget()||preview->parentWidget()->objectName()!="shell")preview->setViewZoom(1.0);};connect(preview,&PreviewWidget::canvasTapped,this,collapseSettings);connect(preview,&PreviewWidget::importRequested,this,&MainWindow::chooseFiles);auto *tapRelay=new TapRelay(collapseSettings,this);mainPreview->installEventFilter(tapRelay);stageHost->installEventFilter(tapRelay);carouselScroll->viewport()->installEventFilter(tapRelay);settingsHost->installEventFilter(tapRelay);
...
void MainWindow::refresh(){
    ...
    zoomValue->setText(QString::number(zoom->value())+"%");if(!currentFile.isEmpty()&&!original.isNull()){QImage composed=original;if(canvasLayers.contains(currentFile)){const auto &layer=canvasLayers[currentFile];if(layer.position!=QPointF()){QImage canvas(QSize(widthInput->value(),heightInput->value()),QImage::Format_ARGB32);canvas.fill(Qt::transparent);QPainter painter(&canvas);painter.drawImage(layer.position,original);composed=canvas;}}preview->setFrame(compositeTimelineTracks(composed));}int variant=previewVariant->currentIndex();preview->setAdjustments(adjustments(),QSize(widthInput->value(),heightInput->value()),variant?&presets[variant-1]:nullptr);
...
void MainWindow::openProjectFile(const QString &path){
    if(galleryScreen)galleryScreen->hide();
    QFile in(path);if(!in.open(QIODevice::ReadOnly)){showError("Could not open project.");return;}const QJsonDocument document=QJsonDocument::fromJson(in.readAll());if(!document.isObject()){showError("This is not a valid Aspectra project.");return;}const QJsonObject root=document.object(),canvas=root.value("canvas").toObject();const int width=canvas.value("width").toInt(1024),height=canvas.value("height").toInt(1024);QStringList sources;canvasLayers.clear();for(const auto &value:root.value("layers").toArray()){const QJsonObject item=value.toObject();const QString source=item.value("source").toString();if(source.isEmpty()||!QFileInfo::exists(source))continue;sources<<source;CanvasLayer layer;layer.source=source;layer.name=item.value("name").toString(QFileInfo(source).completeBaseName());layer.position={item.value("x").toDouble(),item.value("y").toDouble()};layer.nativeSize={item.value("width").toInt(),item.value("height").toInt()};layer.visible=item.value("visible").toBool(true);canvasLayers[source]=layer;}setDimensions(width,height);if(sources.isEmpty()){createNewProject(width,height,QSettings().value("newProjectResolution",300).toInt(),true,QSettings().value("newProjectColorMode","RGB").toString(),QSettings().value("newProjectProfile","sRGB IEC61966-2.1").toString(),Qt::transparent);status->setText("Recovered blank canvas");}else{batch.files.clear();batch.add(sources);updateBatchLabel();selectFile(sources.first());status->setText(QString("Project opened · %1 linked layers").arg(sources.size()));}if(!path.endsWith("Aspectra-X-recovery.aspectra"))rememberRecentDocument(path);
}
#if 0
...
QImage MainWindow::compositeTimelineTracks(const QImage &source) const{
    ...
    return ImageProcessor::compositeTimeline(source,timelineTracks,time);
}
