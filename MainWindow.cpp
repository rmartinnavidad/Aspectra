...
class LayerStreamList final : public QListWidget {
public:
    std::function<void(int)> layerDragStarted;std::function<void()> layerDragFinished;std::function<void(int,int,bool)> layerDropped;explicit LayerStreamList(QWidget *parent=nullptr):QListWidget(parent){}
protected:
    void startDrag(Qt::DropActions) override {auto *item=currentItem();if(!item||item->data(Qt::UserRole+3).toString()!="layer")return;const int trackIndex=item->data(Qt::UserRole+2).toInt();auto *drag=new QDrag(this);auto *mime=new QMimeData;mime->setData("application/x-aspectra-layer-index",QByteArray::number(trackIndex));drag->setMimeData(mime);if(auto *widget=itemWidget(item)){QPixmap pix(widget->size()*devicePixelRatioF());pix.setDevicePixelRatio(devicePixelRatioF());pix.fill(Qt::transparent);widget->render(&pix);drag->setPixmap(pix);drag->setHotSpot(QPoint(pix.width()/pix.devicePixelRatioF()/2,pix.height()/pix.devicePixelRatioF()/2));}if(layerDragStarted)layerDragStarted(trackIndex);drag->exec(Qt::MoveAction);if(layerDragFinished)layerDragFinished();}
    void dragEnterEvent(QDragEnterEvent *event) override {if(event->mimeData()->hasFormat("application/x-aspectra-layer-index")){event->acceptProposedAction();return;}QListWidget::dragEnterEvent(event);}
    void dragMoveEvent(QDragMoveEvent *event) override {if(event->mimeData()->hasFormat("application/x-aspectra-layer-index")){event->acceptProposedAction();return;}QListWidget::dragMoveEvent(event);}
    void dropEvent(QDropEvent *event) override {if(!event->mimeData()->hasFormat("application/x-aspectra-layer-index")){QListWidget::dropEvent(event);return;}const int source=event->mimeData()->data("application/x-aspectra-layer-index").toInt();auto *target=itemAt(event->position().toPoint());if(!target){event->ignore();return;}const QString kind=target->data(Qt::UserRole+3).toString();if(kind!="layer"&&kind!="connector"){event->ignore();return;}const int targetIndex=target->data(Qt::UserRole+2).toInt();const QRect r=visualItemRect(target);const bool after=kind=="layer"&&event->position().x()>r.center().x();if(layerDropped)layerDropped(source,targetIndex,after);event->setDropAction(Qt::MoveAction);event->accept();}
};
class ArtboardDropPopup final : public QFrame {
public:
    std::function<void(int,const QString&)> layerDropped;explicit ArtboardDropPopup(QWidget *parent=nullptr):QFrame(parent){setAcceptDrops(true);}
    void setTargets(const QStringList &ids,const QStringList &names){targets=ids;auto *layout=qobject_cast<QVBoxLayout*>(this->layout());if(!layout){layout=new QVBoxLayout(this);layout->setContentsMargins(6,6,6,6);layout->setSpacing(3);}while(auto *item=layout->takeAt(0)){if(item->widget())delete item->widget();delete item;}for(int i=0;i<ids.size();++i){auto *row=new QLabel(names.value(i),this);row->setProperty("artboardId",ids[i]);row->setFixedHeight(32);row->setAlignment(Qt::AlignVCenter|Qt::AlignLeft);row->setStyleSheet("QLabel{background:#0b0b0f;color:#f4f1fb;border:1px solid #2d2d35;border-radius:7px;padding:0 10px;font-size:10px;font-weight:700;}");layout->addWidget(row);}adjustSize();}
    void clearHover(){for(auto *row:findChildren<QLabel*>())row->setStyleSheet("QLabel{background:#0b0b0f;color:#f4f1fb;border:1px solid #2d2d35;border-radius:7px;padding:0 10px;font-size:10px;font-weight:700;}");}
protected:
    void dragEnterEvent(QDragEnterEvent *event) override {if(event->mimeData()->hasFormat("application/x-aspectra-layer-index"))event->acceptProposedAction();}
    void dragMoveEvent(QDragMoveEvent *event) override {if(!event->mimeData()->hasFormat("application/x-aspectra-layer-index"))return;clearHover();if(auto *row=qobject_cast<QLabel*>(childAt(event->position().toPoint())))row->setStyleSheet("QLabel{background:#171022;color:#ffffff;border:2px solid #7a4dff;border-radius:7px;padding:0 9px;font-size:10px;font-weight:800;}");event->acceptProposedAction();}
    void dragLeaveEvent(QDragLeaveEvent *event) override {clearHover();QFrame::dragLeaveEvent(event);}
    void dropEvent(QDropEvent *event) override {if(!event->mimeData()->hasFormat("application/x-aspectra-layer-index"))return;auto *row=qobject_cast<QLabel*>(childAt(event->position().toPoint()));if(!row){event->ignore();return;}const int track=event->mimeData()->data("application/x-aspectra-layer-index").toInt();const QString id=row->property("artboardId").toString();if(layerDropped&&!id.isEmpty())layerDropped(track,id);event->setDropAction(Qt::MoveAction);event->accept();}
private:QStringList targets;
};

class LayerPoof final : public QWidget {
public:
    explicit LayerPoof(QWidget *parent,const QPixmap &card,const QRect &from,const QRect &to):QWidget(parent),card(card),from(from),to(to){setAttribute(Qt::WA_TransparentForMouseEvents);setAttribute(Qt::WA_TranslucentBackground);const QRect bounds=from.united(to).adjusted(-72,-82,72,72).intersected(parent->rect());origin=bounds.topLeft();this->from.translate(-origin);this->to.translate(-origin);setGeometry(bounds);raise();show();auto *anim=new QVariantAnimation(this);anim->setDuration(560);anim->setStartValue(0.0);anim->setEndValue(1.0);anim->setEasingCurve(QEasingCurve::Linear);QObject::connect(anim,&QVariantAnimation::valueChanged,this,[this](const QVariant &v){progress=v.toReal();update();});QObject::connect(anim,&QVariantAnimation::finished,this,&QObject::deleteLater);anim->start(QAbstractAnimation::DeleteWhenStopped);}
protected:
    void paintEvent(QPaintEvent *) override {QPainter p(this);p.setRenderHint(QPainter::Antialiasing);const qreal t=progress,fly=qMin<qreal>(1.0,t/.76),e=1-std::pow(1-fly,3),pop=qBound<qreal>(0.0,(t-.64)/.36,1.0);const QPointF a=from.center(),b=to.center(),arc(0,-qMin<qreal>(46.0,qAbs(b.x()-a.x())*.10+20.0));const QPointF center=a*(1-e)+b*e+arc*(4*e*(1-e));const qreal scale=1.0-.88*e;QRectF ghost(center.x()-from.width()*scale*.5,center.y()-from.height()*(1.0-.72*e)*.5,qMax<qreal>(8.0,from.width()*scale),qMax<qreal>(6.0,from.height()*(1.0-.72*e)));if(fly<1.0&&!card.isNull()){p.setOpacity(qBound<qreal>(0.0,1.0-e*.72,1.0));p.drawPixmap(ghost,card,card.rect());}p.setOpacity(1.0);const QPointF c=b;for(int i=0;i<14;++i){const qreal angle=6.28318530718*i/14.0+(i%2?.10:-.06),distance=8.0+(i%4)*3.0+46.0*pop,radius=qMax<qreal>(1.0,(6.0-(i%3))*std::pow(1-pop,.72));QColor color=i%3==0?QColor("#7a4dff"):(i%3==1?QColor("#3ddcff"):QColor("#ffffff"));color.setAlphaF(qBound<qreal>(0.0,1.0-pop,1.0));p.setPen(Qt::NoPen);p.setBrush(color);p.drawEllipse(c+QPointF(std::cos(angle)*distance,std::sin(angle)*distance),radius,radius);}if(pop>0){QColor ring("#a98cff");ring.setAlphaF(qBound<qreal>(0.0,.9*(1-pop),1.0));p.setPen(QPen(ring,2.0));p.setBrush(Qt::NoBrush);p.drawEllipse(c,7.0+40.0*pop,7.0+40.0*pop);}}
private: QPixmap card;QRect from,to;QPoint origin;qreal progress=0.0;
};

class LayerActionBurst final : public QWidget {
public:
    enum Role{Mask,Smart,Cleanup,Adjust,Group,Create,Delete};explicit LayerActionBurst(QWidget *parent,const QPoint &center,Role role):QWidget(parent),role(role){setAttribute(Qt::WA_TransparentForMouseEvents);setAttribute(Qt::WA_TranslucentBackground);resize(74,74);move(center-QPoint(37,37));raise();show();auto *anim=new QVariantAnimation(this);anim->setDuration(role==Delete?300:380);anim->setStartValue(0.0);anim->setEndValue(1.0);anim->setEasingCurve(QEasingCurve::OutCubic);QObject::connect(anim,&QVariantAnimation::valueChanged,this,[this](const QVariant &v){t=v.toReal();update();});QObject::connect(anim,&QVariantAnimation::finished,this,&QObject::deleteLater);anim->start(QAbstractAnimation::DeleteWhenStopped);}
protected:
    void paintEvent(QPaintEvent *) override {QPainter p(this);p.setRenderHint(QPainter::Antialiasing);const QPointF c(37,37);QColor primary=role==Delete?QColor("#ff496c"):(role==Cleanup?QColor("#f4d35e"):(role==Mask?QColor("#3ddcff"):QColor("#7a4dff")));primary.setAlphaF(1-t);p.setPen(Qt::NoPen);p.setBrush(primary);if(role==Mask){p.setPen(QPen(primary,3));p.setBrush(Qt::NoBrush);p.drawEllipse(c,8+18*t,8+18*t);p.drawArc(QRectF(c.x()-22,c.y()-8,c.x()?44:44,16),0,180*16);}else if(role==Smart){for(int i=0;i<5;++i){const qreal a=6.28318530718*i/5.0+5*t;p.drawEllipse(c+QPointF(std::cos(a)*(9+17*t),std::sin(a)*(9+17*t)),3.5,3.5);}}else if(role==Cleanup){for(int i=0;i<8;++i)p.drawEllipse(c+QPointF(-18+i*5,14-30*t+(i%2)*5),2.5,2.5);}else if(role==Adjust){p.setPen(QPen(primary,2));for(int i=0;i<8;++i){const qreal a=6.28318530718*i/8.0;p.drawLine(c+QPointF(std::cos(a)*9,std::sin(a)*9),c+QPointF(std::cos(a)*(15+13*t),std::sin(a)*(15+13*t)));}}else if(role==Group){for(int i=0;i<3;++i){QRectF r(c.x()-20+20*t+i*3,c.y()-15+15*t+i*3,28,20);p.drawRoundedRect(r,4,4);}}else if(role==Create){for(int i=0;i<3;++i){QRectF r(c.x()-15,c.y()+15-18*t-i*5,30,20);p.setBrush(QColor(primary.red(),primary.green(),primary.blue(),qMax(0,primary.alpha()-i*35)));p.drawRoundedRect(r,4,4);}}else{for(int i=0;i<10;++i){const qreal a=6.28318530718*i/10.0,d=8+24*t;p.drawRect(QRectF(c+QPointF(std::cos(a)*d,std::sin(a)*d)-QPointF(2,2),QSizeF(4,4)));}}}
private:Role role;qreal t=0;
};

class InlineRenameEdit final : public QLineEdit {
public:
    std::function<void(const QString&)> commitRename;bool finished=false;explicit InlineRenameEdit(QWidget *parent=nullptr):QLineEdit(parent){}
protected:
    void keyPressEvent(QKeyEvent *event) override {if(event->key()==Qt::Key_Escape){finished=true;deleteLater();event->accept();return;}if(event->key()==Qt::Key_Return||event->key()==Qt::Key_Enter){finish();event->accept();return;}QLineEdit::keyPressEvent(event);}
    void focusOutEvent(QFocusEvent *event) override {QLineEdit::focusOutEvent(event);finish();}
private:
    void finish(){if(finished)return;finished=true;const QString value=text().trimmed();if(commitRename&&!value.isEmpty())commitRename(value);deleteLater();}
};
...
QJsonObject trackAsJson(const TimelineTrack &track){
    ...
    const auto &fx=track.style;item["style"]=QJsonObject{{"opacity",fx.opacity},{"effectOpacity",fx.effectOpacity},{"overlay",fx.overlay},{"gradient",fx.gradient},{"stroke",fx.stroke},{"shadow",fx.shadow},{"glow",fx.glow},{"innerShadow",fx.innerShadow},{"bevel",fx.bevel},{"overlayColor",fx.overlayColor.name(QColor::HexArgb)},{"gradientEnd",fx.gradientEnd.name(QColor::HexArgb)},{"strokeColor",fx.strokeColor.name(QColor::HexArgb)},{"shadowColor",fx.shadowColor.name(QColor::HexArgb)},{"glowColor",fx.glowColor.name(QColor::HexArgb)},{"strokeSize",fx.strokeSize},{"shadowSize",fx.shadowSize},{"shadowDistance",fx.shadowDistance},{"glowSize",fx.glowSize},{"bevelSize",fx.bevelSize},{"blendMode",fx.blendMode}};
    if(!track.image.isNull())item["imagePngBase64"]=imageAsBase64(track.image);if(!track.mask.isNull())item["maskPngBase64"]=imageAsBase64(track.mask);
    return item;
}
TimelineTrack trackFromJson(const QJsonObject &item){
    ...
    const auto fx=item.value("style").toObject();if(!fx.isEmpty()){track.style.opacity=fx.value("opacity").toDouble(1);track.style.effectOpacity=fx.value("effectOpacity").toDouble(.75);track.style.overlay=fx.value("overlay").toBool();track.style.gradient=fx.value("gradient").toBool();track.style.stroke=fx.value("stroke").toBool();track.style.shadow=fx.value("shadow").toBool();track.style.glow=fx.value("glow").toBool();track.style.innerShadow=fx.value("innerShadow").toBool();track.style.bevel=fx.value("bevel").toBool();track.style.overlayColor=QColor(fx.value("overlayColor").toString("#ff3d91fb"));track.style.gradientEnd=QColor(fx.value("gradientEnd").toString("#ffca267a"));track.style.strokeColor=QColor(fx.value("strokeColor").toString("#fffac665"));track.style.shadowColor=QColor(fx.value("shadowColor").toString("#ff000000"));track.style.glowColor=QColor(fx.value("glowColor").toString("#ff3d91fb"));track.style.strokeSize=fx.value("strokeSize").toDouble(.01);track.style.shadowSize=fx.value("shadowSize").toDouble(.025);track.style.shadowDistance=fx.value("shadowDistance").toDouble(.025);track.style.glowSize=fx.value("glowSize").toDouble(.025);track.style.bevelSize=fx.value("bevelSize").toDouble(.01);track.style.blendMode=fx.value("blendMode").toInt();}
    track.image=imageFromBase64(item.value("imagePngBase64"));track.mask=imageFromBase64(item.value("maskPngBase64"));return track;
}
...
QIcon settingIcon(const QString &name){
    ...
    if(key=="frame")file="frame tool.png"; else if(key=="canvas")file="icon_canvas.png"; else if(key=="safe")file="icon_safe zone.png"; else if(key=="layer")file="layer stream.png"; else if(key=="mask")file="layer mask.png";
    ...
    else if(key=="smart")file="smart object.png"; else if(key=="cleanup")file="layer cleanup.png"; else if(key=="warp")file="warped text.png"; else if(key=="mirror")file="video editor_flip horizontal.png"; else if(key=="sizes")file="crop tool.png"; else if(key=="images")file="frame tool.png"; else if(key=="videos")file="video.png"; else if(key=="quality")file="anti-aliasing.png"; else if(key=="archive")file="smart object.png";
    return file.isEmpty()?QIcon{}:whiteIcon(root+file);
}
...
void MainWindow::buildUi(){
    ...
    auto *saveProject=button("Save Project…",toolbar,"gold");saveProject->setToolTip("Save linked sources, artboards, and editable masks to an Aspectra project.");
    connect(saveProject,&QPushButton::clicked,this,[this]{
        const QString path=QFileDialog::getSaveFileName(this,"Save Aspectra project","aspectra-project.aspectra","Aspectra project (*.aspectra)");
        if(path.isEmpty())return;
        QJsonObject root;root["version"]=2;root["canvas"]=QJsonObject{{"width",widthInput->value()},{"height",heightInput->value()},{"ratio",ratio->currentText()}};
        QJsonArray layers;
        for(const QString &sourcePath:this->batch.files){
            const CanvasLayer layer=canvasLayers.value(sourcePath);
            QJsonObject item{{"source",sourcePath},{"name",layer.name},{"x",layer.position.x()},{"y",layer.position.y()},{"width",layer.nativeSize.width()},{"height",layer.nativeSize.height()},{"visible",layer.visible},{"opacity",layer.opacity},{"fill",layer.fill},{"blendMode",layer.blendMode},{"lockPosition",layer.lockPosition},{"spaceId",layer.spaceId}};
    ...
    connect(subButtons,&QButtonGroup::idClicked,this,[this,settingsHost,settingCard,settingCardLayout,subNames](int index){
        QTimer::singleShot(0,this,[this,settingsHost,settingCard,settingCardLayout,subNames,index]{
           ...
           if (name == "Layer") {
                ...
                auto *railWidget = new LayerStreamList;
                ...
                railWidget->setDragDropMode(QAbstractItemView::DragDrop);
                ...
                *populateRailPtr = [this, railWidget, backBtn, crumbLabel, stackWidget, blendCombo, showArtboardWorkspace, syncLayerControls, thumbnailIcon, expandedArtboards, selectedStreamTrack, animateStreamArtboards]() {
                    ...
                    if(this->batch.files.isEmpty()){auto *item=new QListWidgetItem("Blank Canvas");item->setData(Qt::UserRole,QString());railWidget->addItem(item);}else{
                        for(const QString &path:spaceArtboards()){
                            ...
                            auto *streamIcon=new QLabel(card);streamIcon->setPixmap(whiteIcon("C:/Users/rmart/Reign of Glory/blender/00 Addons/custom add ons/utilities/ASPECTRA/tools/layer stream.png").pixmap(18,18));streamIcon->setFixedSize(22,30);streamIcon->setAlignment(Qt::AlignCenter);streamIcon->setToolTip(expanded?"Close Layer Stream":"Open Layer Stream");streamIcon->setStyleSheet("background:transparent;border:0;");
                            auto *layerWheel=new QLabel(QString::number(layerCount),card);layerWheel->setFixedSize(30,30);layerWheel->setAlignment(Qt::AlignCenter);layerWheel->setToolTip(QString("%1 layer%2 in this artboard · click to open stream").arg(layerCount).arg(layerCount==1?"":"s"));layerWheel->setStyleSheet("color:#ffffff;background:transparent;border:0;font-size:11px;font-weight:700;");
                            cardRow->addWidget(thumb);cardRow->addWidget(nameLabel,1);cardRow->addWidget(streamIcon);cardRow->addWidget(layerWheel);railWidget->setItemWidget(item,card);if(active){item->setSelected(true);railWidget->setCurrentItem(item);}
                            if(expanded){int streamPosition=0;QVector<int> owned;for(int i=0;i<timelineTracks.size();++i)if(timelineTracks[i].artboardSource==path)owned.append(i);for(int ownedPos=0;ownedPos<owned.size();++ownedPos){const int i=owned[ownedPos];const auto &track=timelineTracks[i];const bool selected=*selectedStreamTrack==i;QString tName=track.name.isEmpty()?QString("Layer %1").arg(i+1):track.name;
                                auto *connectorItem=new QListWidgetItem;connectorItem->setData(Qt::UserRole,path);connectorItem->setData(Qt::UserRole+2,i);connectorItem->setData(Qt::UserRole+3,"connector");connectorItem->setFlags(Qt::ItemIsEnabled);connectorItem->setSizeHint(QSize(28,selected?64:56));railWidget->addItem(connectorItem);auto *connectorBlock=new QWidget(railWidget);connectorBlock->setAttribute(Qt::WA_TransparentForMouseEvents);connectorBlock->setStyleSheet("background:transparent;border:0;");auto *connectorLayout=new QHBoxLayout(connectorBlock);connectorLayout->setContentsMargins(0,0,0,0);connectorLayout->setSpacing(0);auto *connector=new QLabel(connectorBlock);connector->setPixmap(whiteIcon("C:/Users/rmart/Reign of Glory/blender/00 Addons/custom add ons/utilities/ASPECTRA/tools/stream connector.png").pixmap(24,16));connector->setAlignment(Qt::AlignCenter);connector->setStyleSheet("background:transparent;border:0;");connectorLayout->addWidget(connector);railWidget->setItemWidget(connectorItem,connectorBlock);
                                const bool hasMask=!track.mask.isNull(),hasFx=track.style.hasStyles();auto *layerItem=new QListWidgetItem;layerItem->setData(Qt::UserRole,path);layerItem->setData(Qt::UserRole+2,i);layerItem->setData(Qt::UserRole+3,"layer");layerItem->setSizeHint(QSize(hasMask?236:202,selected?64:56));layerItem->setFlags(Qt::ItemIsEnabled|Qt::ItemIsSelectable|Qt::ItemIsDragEnabled);railWidget->addItem(layerItem);
                                auto *layerCard=new QWidget(railWidget);layerCard->setAttribute(Qt::WA_TransparentForMouseEvents);layerCard->setStyleSheet(QString("QWidget{background:%1;border:1px solid %2;border-radius:8px;}").arg(selected?"#171022":"#0b0b0f",selected?"#7a4dff":"#292331"));auto *layerRow=new QHBoxLayout(layerCard);layerRow->setContentsMargins(5,3,6,3);layerRow->setSpacing(4);
                                auto *visibility=new QLabel(layerCard);visibility->setPixmap(whiteIcon(QString("C:/Users/rmart/Reign of Glory/blender/00 Addons/custom add ons/utilities/ASPECTRA/tools/")+(track.enabled?"layer_show.png":"layer_hidden.png")).pixmap(16,16));visibility->setFixedSize(22,22);visibility->setAlignment(Qt::AlignCenter);visibility->setToolTip(track.enabled?"Hide layer":"Show layer");visibility->setStyleSheet("background:transparent;border:0;");
                                ...
                                auto *layerThumb=new QLabel(layerCard);if(!layerImage.isNull())layerThumb->setPixmap(thumbnailIcon(layerImage,selected?42:36).pixmap(selected?42:36,selected?42:36));else{const QString typeIcon=track.isGroup?"group.png":(track.type==TimelineTrack::Effect?"add effects.png":(track.type==TimelineTrack::Text?"horizontal type tool.png":"add layer.png"));layerThumb->setPixmap(whiteIcon("C:/Users/rmart/Reign of Glory/blender/00 Addons/custom add ons/utilities/ASPECTRA/tools/"+typeIcon).pixmap(selected?28:24,selected?28:24));}layerThumb->setFixedSize(selected?42:36,selected?42:36);layerThumb->setAlignment(Qt::AlignCenter);layerThumb->setStyleSheet(QString("background:#11151a;border:2px solid %1;border-radius:5px;").arg(selected?"#7a4dff":"#34303d"));
                                QLabel *maskThumb=nullptr;if(hasMask){maskThumb=new QLabel(layerCard);maskThumb->setPixmap(thumbnailIcon(track.mask,selected?42:36).pixmap(selected?42:36,selected?42:36));maskThumb->setFixedSize(selected?42:36,selected?42:36);maskThumb->setAlignment(Qt::AlignCenter);maskThumb->setToolTip("Layer mask");maskThumb->setStyleSheet("background:#11151a;border:1px solid #55515f;border-radius:5px;");}
                                auto *layerText=new QWidget(layerCard);layerText->setStyleSheet("background:transparent;border:0;");auto *textCol=new QVBoxLayout(layerText);textCol->setContentsMargins(0,0,0,0);textCol->setSpacing(0);textCol->setAlignment(Qt::AlignVCenter);auto *layerName=new QLabel(tName,layerText);layerName->setMinimumWidth(62);layerName->setMaximumWidth(82);layerName->setStyleSheet("color:#f2eff8;font-size:9px;font-weight:700;background:transparent;border:0;");layerName->setToolTip(tName);const QString kind=track.isGroup?"GROUP":(track.type==TimelineTrack::Text?"TYPE":(track.type==TimelineTrack::Effect?"EFFECT":(track.smartObject?"SMART OBJECT":"LAYER")));auto *kindLabel=new QLabel(kind,layerText);kindLabel->setStyleSheet(QString("color:%1;font-size:7px;font-weight:700;background:transparent;border:0;").arg(selected?"#a98cff":"#706a7a"));textCol->addWidget(layerName);textCol->addWidget(kindLabel);
                                layerRow->addWidget(visibility);layerRow->addWidget(layerThumb);if(maskThumb)layerRow->addWidget(maskThumb);layerRow->addWidget(layerText,1);if(hasFx){auto *fxLabel=new QLabel("fx",layerCard);fxLabel->setFixedSize(22,22);fxLabel->setAlignment(Qt::AlignCenter);fxLabel->setToolTip("Layer Style");fxLabel->setStyleSheet("color:#f39c12;font-size:9px;font-weight:800;background:transparent;border:0;");layerRow->addWidget(fxLabel);}railWidget->setItemWidget(layerItem,layerCard);
                                if(animateStreamArtboards->contains(path)){for(QWidget *animated:{connectorBlock,layerCard}){auto *fade=new QGraphicsOpacityEffect(animated);animated->setGraphicsEffect(fade);fade->setOpacity(0.0);auto *anim=new QPropertyAnimation(fade,"opacity",animated);anim->setDuration(190);anim->setStartValue(0.0);anim->setEndValue(1.0);anim->setEasingCurve(QEasingCurve::OutBack);QTimer::singleShot(qMin(streamPosition*24,120),animated,[anim]{anim->start(QAbstractAnimation::DeleteWhenStopped);});}}++streamPosition;
                            }}
                        }
                    }
                ...
                *populateLayersPtr = [this, detailRowList, backBtn, crumbLabel, stackWidget, blendCombo, syncLayerControls, thumbnailIcon](const QString &sourcePath) {
                    ...
                    for (int i = 0; i < timelineTracks.size(); ++i) {
                        ...
                        auto *trackWidget = createLayerRow(tName, track.image, track.mask, false, track.enabled, track.style.hasStyles(), track.smartObject, [this, i](bool visible) {
                ...
                auto *artboardDropPopup=new ArtboardDropPopup(settingCard);artboardDropPopup->setObjectName("LayerArtboardDropPopup");artboardDropPopup->hide();artboardDropPopup->setStyleSheet("QFrame#LayerArtboardDropPopup{background:#050506;border:1px solid #7a4dff;border-radius:10px;}");
                auto hideArtboardDropPopup=[artboardDropPopup](){if(!artboardDropPopup->isVisible())return;auto *fade=qobject_cast<QGraphicsOpacityEffect*>(artboardDropPopup->graphicsEffect());if(!fade){fade=new QGraphicsOpacityEffect(artboardDropPopup);artboardDropPopup->setGraphicsEffect(fade);}auto *anim=new QPropertyAnimation(fade,"opacity",artboardDropPopup);anim->setDuration(120);anim->setStartValue(fade->opacity());anim->setEndValue(0.0);anim->setEasingCurve(QEasingCurve::InCubic);QObject::connect(anim,&QPropertyAnimation::finished,artboardDropPopup,[artboardDropPopup]{artboardDropPopup->hide();artboardDropPopup->clearHover();});anim->start(QAbstractAnimation::DeleteWhenStopped);};
                auto showArtboardDropPopup=[this,settingCard,railWidget,artboardDropPopup](int sourceTrack){if(sourceTrack<0||sourceTrack>=timelineTracks.size())return;QStringList ids,names;for(const QString &id:this->batch.files){ids<<id;QString name=canvasLayers.value(id).name;if(name.isEmpty())name=QFileInfo(id).completeBaseName();if(name.isEmpty())name="Artboard";names<<((id==timelineTracks[sourceTrack].artboardSource?QString::fromUtf8("● "):QString::fromUtf8("○ "))+name);}artboardDropPopup->setTargets(ids,names);const int h=qMin(220,qMax(44,ids.size()*35+12)),w=qMin(260,qMax(180,railWidget->width()/2));const QPoint railTop=railWidget->mapTo(settingCard,QPoint(0,0));const int x=qBound(4,railTop.x()+railWidget->width()-w-8,qMax(4,settingCard->width()-w-4)),y=qMax(4,railTop.y()-h-4);artboardDropPopup->setGeometry(x,y,w,h);artboardDropPopup->raise();auto *fade=qobject_cast<QGraphicsOpacityEffect*>(artboardDropPopup->graphicsEffect());if(!fade){fade=new QGraphicsOpacityEffect(artboardDropPopup);artboardDropPopup->setGraphicsEffect(fade);}fade->setOpacity(0.0);artboardDropPopup->show();const QPoint finalPos=artboardDropPopup->pos();artboardDropPopup->move(finalPos+QPoint(0,12));auto *move=new QPropertyAnimation(artboardDropPopup,"pos",artboardDropPopup);move->setDuration(170);move->setStartValue(artboardDropPopup->pos());move->setEndValue(finalPos);move->setEasingCurve(QEasingCurve::OutBack);move->start(QAbstractAnimation::DeleteWhenStopped);auto *opacity=new QPropertyAnimation(fade,"opacity",artboardDropPopup);opacity->setDuration(130);opacity->setStartValue(0.0);opacity->setEndValue(1.0);opacity->setEasingCurve(QEasingCurve::OutCubic);opacity->start(QAbstractAnimation::DeleteWhenStopped);};
                railWidget->layerDragStarted=[showArtboardDropPopup](int trackIndex){showArtboardDropPopup(trackIndex);};railWidget->layerDragFinished=[hideArtboardDropPopup](){hideArtboardDropPopup();};
                railWidget->layerDropped=[this,railWidget,stackWidget,populateRailPtr,populateLayersPtr,selectedStreamTrack,animateStreamArtboards](int source,int target,bool after){if(source<0||source>=timelineTracks.size()||target<0||target>=timelineTracks.size()||source==target)return;TimelineTrack moving=timelineTracks[source];if(moving.locked||moving.lockPosition){status->setText("Layer is locked");return;}const QString sourceBoard=moving.artboardSource,targetBoard=timelineTracks[target].artboardSource;timelineTracks.removeAt(source);int adjustedTarget=target-(source<target?1:0);int insertAt=qBound(0,adjustedTarget+(after?1:0),int(timelineTracks.size()));moving.artboardSource=targetBoard;timelineTracks.insert(insertAt,moving);*selectedStreamTrack=insertAt;animateStreamArtboards->insert(sourceBoard);animateStreamArtboards->insert(targetBoard);updateTrackPanel();refresh();autosaveProject();if(populateRailPtr)(*populateRailPtr)();if(populateLayersPtr&&stackWidget->currentIndex()==1)(*populateLayersPtr)(currentFile);for(int i=0;i<railWidget->count();++i)if(railWidget->item(i)->data(Qt::UserRole+3).toString()=="layer"&&railWidget->item(i)->data(Qt::UserRole+2).toInt()==insertAt){railWidget->setCurrentRow(i);railWidget->scrollToItem(railWidget->item(i),QAbstractItemView::EnsureVisible);break;}status->setText(sourceBoard==targetBoard?"Layer reordered":"Layer moved to another artboard");};
                artboardDropPopup->layerDropped=[this,settingCard,railWidget,artboardDropPopup,hideArtboardDropPopup,populateRailPtr,populateLayersPtr,selectedStreamTrack,animateStreamArtboards](int source,const QString &targetBoard){if(source<0||source>=timelineTracks.size()||!this->batch.files.contains(targetBoard))return;TimelineTrack moving=timelineTracks[source];if(moving.locked||moving.lockPosition){status->setText("Layer is locked");hideArtboardDropPopup();return;}const QString sourceBoard=moving.artboardSource;if(sourceBoard==targetBoard){hideArtboardDropPopup();return;}QPixmap card;QRect from;for(int n=0;n<railWidget->count();++n){auto *item=railWidget->item(n);if(item->data(Qt::UserRole+3).toString()=="layer"&&item->data(Qt::UserRole+2).toInt()==source){if(auto *widget=railWidget->itemWidget(item)){card=widget->grab();const QPoint topLeft=widget->mapTo(settingCard,QPoint());from=QRect(topLeft,widget->size());widget->hide();}break;}}QRect to=artboardDropPopup->geometry();for(auto *row:artboardDropPopup->findChildren<QLabel*>())if(row->property("artboardId").toString()==targetBoard){const QPoint topLeft=row->mapTo(settingCard,QPoint());to=QRect(topLeft,row->size());break;}timelineTracks.removeAt(source);int insertAt=timelineTracks.size();for(int i=timelineTracks.size()-1;i>=0;--i)if(timelineTracks[i].artboardSource==targetBoard){insertAt=i+1;break;}moving.artboardSource=targetBoard;timelineTracks.insert(insertAt,moving);*selectedStreamTrack=insertAt;animateStreamArtboards->insert(sourceBoard);animateStreamArtboards->insert(targetBoard);if(from.isValid()&&!card.isNull())new LayerPoof(settingCard,card,from,to);auto *fade=qobject_cast<QGraphicsOpacityEffect*>(artboardDropPopup->graphicsEffect());if(fade){auto *flash=new QPropertyAnimation(fade,"opacity",artboardDropPopup);flash->setDuration(360);flash->setKeyValueAt(0.0,1.0);flash->setKeyValueAt(.35,.28);flash->setKeyValueAt(.70,1.0);flash->setKeyValueAt(1.0,.82);flash->setEasingCurve(QEasingCurve::OutCubic);flash->start(QAbstractAnimation::DeleteWhenStopped);}QTimer::singleShot(620,this,[this,targetBoard,populateRailPtr,populateLayersPtr,hideArtboardDropPopup](){updateTrackPanel();refresh();autosaveProject();if(populateRailPtr)(*populateRailPtr)();if(populateLayersPtr&&currentFile==targetBoard)(*populateLayersPtr)(targetBoard);hideArtboardDropPopup();status->setText("Layer tucked into "+canvasLayers.value(targetBoard).name);});};
                ...
                auto showArtboardContextMenu = [this, railWidget, detailRowList, populateRailPtr, populateLayersPtr, createArtboardAfter, deleteArtboard, renderArtboard](const QString &contextArtboardId, const QPoint &globalPosition) {
                    ...
                    if(!hasMask)addMask=menu.addAction(whiteIcon("C:/Users/rmart/Reign of Glory/blender/00 Addons/custom add ons/utilities/ASPECTRA/tools/layer mask.png"),"Add Layer Mask");
                    else{editMask=menu.addAction(whiteIcon("C:/Users/rmart/Reign of Glory/blender/00 Addons/custom add ons/utilities/ASPECTRA/tools/layer mask.png"),"Edit Layer Mask");menu.addSeparator();invertMask=menu.addAction("Invert Layer Mask");clearMaskWhite=menu.addAction("Clear Layer Mask to White");clearMaskBlack=menu.addAction("Clear Layer Mask to Black");menu.addSeparator();deleteMask=menu.addAction("Delete Layer Mask");}
                    menu.addSeparator();
                ...
                auto renameStreamEntity=[this,populateRailPtr,populateLayersPtr](QListWidget *list,QListWidgetItem *item,bool detailed){if(!list||!item)return;const QString kind=detailed?item->data(Qt::UserRole).toString():item->data(Qt::UserRole+3).toString();const QString path=detailed?(kind=="base"?item->data(Qt::UserRole+1).toString():currentFile):item->data(Qt::UserRole).toString();const int trackIndex=item->data(Qt::UserRole+2).toInt();QString oldName;if(kind=="artboard"||kind=="base")oldName=canvasLayers.value(path).name;else if((kind=="layer"||kind=="track")&&trackIndex>=0&&trackIndex<timelineTracks.size())oldName=timelineTracks[trackIndex].name;else return;QRect r=list->visualItemRect(item);int left=detailed?62:(kind=="artboard"?70:68);int right=detailed?qMax(left+100,r.width()-72):(kind=="artboard"?205:r.width()-5);auto *edit=new InlineRenameEdit(list->viewport());edit->setText(oldName);edit->setGeometry(r.left()+left,r.top()+4,qMax(80,right-left),qMax(26,r.height()-8));edit->setStyleSheet("QLineEdit{background:#0b0b0f;color:#ffffff;border:2px solid #3ddcff;border-radius:5px;padding:2px 6px;font-weight:700;}");edit->commitRename=[this,kind,path,trackIndex,oldName,populateRailPtr,populateLayersPtr,detailed](const QString &name){if(name==oldName)return;if(kind=="artboard"||kind=="base")canvasLayers[path].name=name;else if(trackIndex>=0&&trackIndex<timelineTracks.size())timelineTracks[trackIndex].name=name;updateTrackPanel();if(populateRailPtr)(*populateRailPtr)();if(detailed&&populateLayersPtr)(*populateLayersPtr)(path);autosaveProject();status->setText("Renamed · "+name);};edit->show();edit->setFocus(Qt::MouseFocusReason);edit->selectAll();};
                auto showTrackProperties=[this,populateRailPtr,populateLayersPtr](int trackIndex){if(trackIndex<0||trackIndex>=timelineTracks.size())return;TimelineTrack draft=timelineTracks[trackIndex];QDialog dialog(this);dialog.setWindowTitle((draft.isGroup?"Group":"Layer")+QString(" Properties · ")+draft.name);dialog.resize(430,610);auto *outer=new QVBoxLayout(&dialog);outer->setContentsMargins(14,14,14,14);auto *tabsBox=new QTabWidget(&dialog);outer->addWidget(tabsBox,1);auto *general=new QWidget;auto *generalForm=new QFormLayout(general);auto *nameEdit=new QLineEdit(draft.name);generalForm->addRow("Name",nameEdit);auto *blend=new QComboBox;blend->addItems({"Normal","Multiply","Screen","Overlay","Color Dodge","Lighten"});blend->setCurrentText(draft.blendMode);generalForm->addRow("Blend mode",blend);auto *opacity=new QSpinBox;opacity->setRange(0,100);opacity->setSuffix(" %");opacity->setValue(qRound(draft.opacity*100));generalForm->addRow("Opacity",opacity);auto *fill=new QSpinBox;fill->setRange(0,100);fill->setSuffix(" %");fill->setValue(qRound(draft.fill*100));generalForm->addRow("Fill",fill);auto *visible=new QCheckBox("Visible");visible->setChecked(draft.enabled);generalForm->addRow(visible);auto *lockPixelsBox=new QCheckBox("Lock pixels");lockPixelsBox->setChecked(draft.lockPixels);generalForm->addRow(lockPixelsBox);auto *lockPositionBox=new QCheckBox("Lock position");lockPositionBox->setChecked(draft.lockPosition);generalForm->addRow(lockPositionBox);auto *lockAllBox=new QCheckBox("Lock all");lockAllBox->setChecked(draft.locked);generalForm->addRow(lockAllBox);tabsBox->addTab(general,"Properties");
                    auto *styles=new QWidget;auto *styleLayout=new QVBoxLayout(styles);styleLayout->setSpacing(7);styleLayout->addWidget(label("LAYER STYLE","section"));auto *effectOpacity=new QSpinBox;effectOpacity->setRange(0,100);effectOpacity->setSuffix(" %");effectOpacity->setValue(qRound(draft.style.effectOpacity*100));auto *effectLine=new QHBoxLayout;effectLine->addWidget(label("Effect opacity"));effectLine->addWidget(effectOpacity);styleLayout->addLayout(effectLine);const QStringList effectNames{"Color Overlay","Gradient Overlay","Stroke","Drop Shadow","Outer Glow","Inner Shadow","Bevel / Emboss"};QVector<QCheckBox*> effects;for(const QString &effectName:effectNames){auto *check=new QCheckBox(effectName);styleLayout->addWidget(check);effects.append(check);}effects[0]->setChecked(draft.style.overlay);effects[1]->setChecked(draft.style.gradient);effects[2]->setChecked(draft.style.stroke);effects[3]->setChecked(draft.style.shadow);effects[4]->setChecked(draft.style.glow);effects[5]->setChecked(draft.style.innerShadow);effects[6]->setChecked(draft.style.bevel);auto addSize=[&](const QString &name,double value){auto *line=new QHBoxLayout;line->addWidget(label(name));auto *spin=new QDoubleSpinBox;spin->setRange(.1,20);spin->setDecimals(1);spin->setSuffix(" %");spin->setValue(value*100);line->addWidget(spin);styleLayout->addLayout(line);return spin;};auto *strokeSize=addSize("Stroke width",draft.style.strokeSize);auto *shadowSize=addSize("Shadow softness",draft.style.shadowSize);auto *shadowDistance=addSize("Shadow distance",draft.style.shadowDistance);auto *glowSize=addSize("Glow radius",draft.style.glowSize);auto *bevelSize=addSize("Bevel size",draft.style.bevelSize);auto addColor=[&](const QString &name,QColor initial,QColor *target){auto *button=new QPushButton(name+"…");button->setStyleSheet(QString("QPushButton{border-left:12px solid %1;}").arg(initial.name()));QObject::connect(button,&QPushButton::clicked,&dialog,[this,button,target]{QColor chosen=QColorDialog::getColor(*target,this);if(chosen.isValid()){*target=chosen;button->setStyleSheet(QString("QPushButton{border-left:12px solid %1;}").arg(chosen.name()));}});styleLayout->addWidget(button);};addColor("Overlay color",draft.style.overlayColor,&draft.style.overlayColor);addColor("Gradient end",draft.style.gradientEnd,&draft.style.gradientEnd);addColor("Stroke color",draft.style.strokeColor,&draft.style.strokeColor);addColor("Shadow color",draft.style.shadowColor,&draft.style.shadowColor);addColor("Glow color",draft.style.glowColor,&draft.style.glowColor);styleLayout->addStretch();tabsBox->addTab(styles,"Layer Style");auto *buttons=new QDialogButtonBox(QDialogButtonBox::Ok|QDialogButtonBox::Cancel);outer->addWidget(buttons);QObject::connect(buttons,&QDialogButtonBox::accepted,&dialog,&QDialog::accept);QObject::connect(buttons,&QDialogButtonBox::rejected,&dialog,&QDialog::reject);if(dialog.exec()!=QDialog::Accepted)return;draft.name=nameEdit->text().trimmed().isEmpty()?draft.name:nameEdit->text().trimmed();draft.blendMode=blend->currentText();draft.opacity=opacity->value()/100.;draft.fill=fill->value()/100.;draft.enabled=visible->isChecked();draft.lockPixels=lockPixelsBox->isChecked();draft.lockPosition=lockPositionBox->isChecked();draft.locked=lockAllBox->isChecked();draft.style.effectOpacity=effectOpacity->value()/100.;draft.style.overlay=effects[0]->isChecked();draft.style.gradient=effects[1]->isChecked();draft.style.stroke=effects[2]->isChecked();draft.style.shadow=effects[3]->isChecked();draft.style.glow=effects[4]->isChecked();draft.style.innerShadow=effects[5]->isChecked();draft.style.bevel=effects[6]->isChecked();draft.style.strokeSize=strokeSize->value()/100.;draft.style.shadowSize=shadowSize->value()/100.;draft.style.shadowDistance=shadowDistance->value()/100.;draft.style.glowSize=glowSize->value()/100.;draft.style.bevelSize=bevelSize->value()/100.;timelineTracks[trackIndex]=draft;updateTrackPanel();refresh();autosaveProject();if(populateRailPtr)(*populateRailPtr)();if(populateLayersPtr&&!draft.artboardSource.isEmpty())(*populateLayersPtr)(draft.artboardSource);status->setText("Layer properties updated · "+draft.name);};
                ...
                QObject::connect(detailRowList->viewport(), &QWidget::customContextMenuRequested, detailRowList,
                    ...
                    if (!item) return;
                    const bool trackRow = item->data(Qt::UserRole).toString() == "track";
                    const int trackIndex = trackRow ? item->data(Qt::UserRole + 2).toInt() : -1;
                    if (trackRow && (trackIndex < 0 || trackIndex >= timelineTracks.size())) return;
                    const QString sourcePath = currentFile;
                    QMenu menu(detailRowList);
                    auto *rename = menu.addAction(trackRow ? "Rename Layer" : "Rename Artboard");
                    QAction *duplicate = trackRow ? menu.addAction("Duplicate Layer") : nullptr;
                    QAction *mask = !trackRow ? menu.addAction(canvasLayers.value(sourcePath).mask.isNull() ? "Add Mask" : "Alt+Click Mask to Edit") : nullptr;
                    auto *remove = trackRow ? menu.addAction("Delete Layer") : nullptr;
                    QAction *chosen = menu.exec(detailRowList->viewport()->mapToGlobal(position));
                    if (!chosen) return;
                    if (chosen == rename) {
                        bool ok = false;
                        const QString oldName = trackRow ? timelineTracks[trackIndex].name : canvasLayers.value(sourcePath).name;
                        const QString name = QInputDialog::getText(this, "Rename", "Name", QLineEdit::Normal, oldName, &ok).trimmed();
                        if (ok && !name.isEmpty()) {
                            if (trackRow) timelineTracks[trackIndex].name = name;else canvasLayers[sourcePath].name = name;
                            (*populateLayersPtr)(sourcePath);autosaveProject();
                        }
                    } else if (chosen == duplicate) {
                        timelineTracks.insert(trackIndex + 1, timelineTracks[trackIndex]);
                        timelineTracks[trackIndex + 1].name += " copy";
                        updateTrackPanel();refresh();(*populateLayersPtr)(sourcePath);autosaveProject();
                    } else if (chosen == mask) {
                        auto &layer = canvasLayers[sourcePath];
                        if (layer.mask.isNull()) { layer.mask = QImage(layer.nativeSize.isValid() ? layer.nativeSize : original.size(), QImage::Format_Grayscale8);layer.mask.fill(255); }
                        preview->setLayerMask(layer.mask);preview->setMaskEditMode(true);
                        (*populateLayersPtr)(sourcePath);
                        status->setText("Editing mask · drag to reveal · Shift+drag to hide");
                    } else if (chosen == remove) {
                        timelineTracks.removeAt(trackIndex);updateTrackPanel();refresh();(*populateLayersPtr)(sourcePath);autosaveProject();
                    }
                });

                QObject::connect(detailRowList,&QListWidget::itemDoubleClicked,this,[this,detailRowList,renameStreamEntity,showTrackProperties,showArtboardContextMenu](QListWidgetItem *item){if(!item)return;const QRect r=detailRowList->visualItemRect(item);const int localX=detailRowList->viewport()->mapFromGlobal(QCursor::pos()).x()-r.left();const QString kind=item->data(Qt::UserRole).toString();if(localX>=62&&localX<=qMax(220,r.width()-80)){renameStreamEntity(detailRowList,item,true);return;}if(kind=="track")showTrackProperties(item->data(Qt::UserRole+2).toInt());else if(kind=="base"){const QString path=item->data(Qt::UserRole+1).toString();if(!path.isEmpty())showArtboardContextMenu(path,QCursor::pos());}});
                auto *railRenameReturn=new QShortcut(QKeySequence(Qt::Key_Return),railWidget);QObject::connect(railRenameReturn,&QShortcut::activated,railWidget,[railWidget,renameStreamEntity]{renameStreamEntity(railWidget,railWidget->currentItem(),false);});auto *railRenameEnter=new QShortcut(QKeySequence(Qt::Key_Enter),railWidget);QObject::connect(railRenameEnter,&QShortcut::activated,railWidget,[railWidget,renameStreamEntity]{renameStreamEntity(railWidget,railWidget->currentItem(),false);});auto *detailRenameReturn=new QShortcut(QKeySequence(Qt::Key_Return),detailRowList);QObject::connect(detailRenameReturn,&QShortcut::activated,detailRowList,[detailRowList,renameStreamEntity]{renameStreamEntity(detailRowList,detailRowList->currentItem(),true);});auto *detailRenameEnter=new QShortcut(QKeySequence(Qt::Key_Enter),detailRowList);QObject::connect(detailRenameEnter,&QShortcut::activated,detailRowList,[detailRowList,renameStreamEntity]{renameStreamEntity(detailRowList,detailRowList->currentItem(),true);});
                QObject::connect(railWidget,&QListWidget::itemDoubleClicked,this,[this,railWidget,populateLayersPtr,renameStreamEntity,showTrackProperties](QListWidgetItem *item){if(!item)return;const QString kind=item->data(Qt::UserRole+3).toString();const QRect r=railWidget->visualItemRect(item);const int localX=railWidget->viewport()->mapFromGlobal(QCursor::pos()).x()-r.left();if(kind=="artboard"){if(localX>=52&&localX<=174){renameStreamEntity(railWidget,item,false);return;}const QString path=item->data(Qt::UserRole).toString();if(populateLayersPtr&&!path.isEmpty())(*populateLayersPtr)(path);return;}if(kind=="layer"){const int trackIndex=item->data(Qt::UserRole+2).toInt();if(localX>=70){renameStreamEntity(railWidget,item,false);return;}showTrackProperties(trackIndex);}});
                ...
                for (auto *btn : {maskBtn, smartBtn, cleanupBtn, adjBtn, grpBtn, newLayerBtn, delBtn}) {
                    btn->setFixedSize(28, 28);btn->setIconSize(QSize(15, 15));btn->setStyleSheet("QToolButton { background: #000000; border: 1px solid #2d2d35; border-radius: 6px; } QToolButton:hover { background: #111114; border-color: #3ddcff; }");actionRow->addWidget(btn);
                }
                auto animateAction=[settingCard](QToolButton *button,LayerActionBurst::Role role){const QPoint center=button->mapTo(settingCard,QPoint(button->width()/2,button->height()/2));new LayerActionBurst(settingCard,center,role);auto *press=new QPropertyAnimation(button,"iconSize",button);press->setDuration(180);press->setStartValue(QSize(11,11));press->setKeyValueAt(.55,QSize(19,19));press->setEndValue(QSize(15,15));press->setEasingCurve(QEasingCurve::OutBack);press->start(QAbstractAnimation::DeleteWhenStopped);};QObject::connect(maskBtn,&QToolButton::clicked,settingCard,[=]{animateAction(maskBtn,LayerActionBurst::Mask);});QObject::connect(smartBtn,&QToolButton::clicked,settingCard,[=]{animateAction(smartBtn,LayerActionBurst::Smart);});QObject::connect(cleanupBtn,&QToolButton::clicked,settingCard,[=]{animateAction(cleanupBtn,LayerActionBurst::Cleanup);});QObject::connect(adjBtn,&QToolButton::clicked,settingCard,[=]{animateAction(adjBtn,LayerActionBurst::Adjust);});QObject::connect(grpBtn,&QToolButton::clicked,settingCard,[=]{animateAction(grpBtn,LayerActionBurst::Group);});QObject::connect(newLayerBtn,&QToolButton::clicked,settingCard,[=]{animateAction(newLayerBtn,LayerActionBurst::Create);});QObject::connect(delBtn,&QToolButton::clicked,settingCard,[=]{animateAction(delBtn,LayerActionBurst::Delete);});
...
void MainWindow::ensureSpaces(){if(spaces.isEmpty()){AspectraSpace space;space.id="space-"+QUuid::createUuid().toString(QUuid::Id128).left(12);space.name="Space 1";spaces.append(space);}if(activeSpaceId.isEmpty()||std::none_of(spaces.cbegin(),spaces.cend(),[this](const AspectraSpace &space){return space.id==activeSpaceId;}))activeSpaceId=spaces.first().id;for(auto it=canvasLayers.begin();it!=canvasLayers.end();++it)if(it->spaceId.isEmpty())it->spaceId=activeSpaceId;}
QStringList MainWindow::spaceArtboards(const QString &spaceId) const {const QString id=spaceId.isEmpty()?activeSpaceId:spaceId;QStringList result;for(const QString &source:batch.files)if(canvasLayers.value(source).spaceId==id)result<<source;return result;}
void MainWindow::rebuildSpaceSelector(){if(!spaceSelector)return;QSignalBlocker block(spaceSelector);spaceSelector->clear();for(const auto &space:spaces)spaceSelector->addItem(space.name,space.id);spaceSelector->setCurrentIndex(qMax(0,spaceSelector->findData(activeSpaceId)));}
void MainWindow::setActiveSpace(const QString &id,bool selectArtboard){auto it=std::find_if(spaces.begin(),spaces.end(),[&](const AspectraSpace &space){return space.id==id;});if(it==spaces.end()||activeSpaceId==id){rebuildSpaceSelector();return;}for(auto &space:spaces)if(space.id==activeSpaceId){space.activeArtboard=currentFile;break;}activeSpaceId=id;rebuildSpaceSelector();const QStringList boards=spaceArtboards();if(selectArtboard&&!boards.isEmpty()){QString target=it->activeArtboard;if(!boards.contains(target))target=boards.first();selectFile(target);}else if(preview)preview->clearArtboards();autosaveProject();status->setText(QString("Space · %1 · %2 artboards").arg(it->name).arg(boards.size()));}
...
void MainWindow::createNewProject(int width,int height,int resolution,bool artboard,const QString &colorMode,const QString &profile,const QColor &background){
    ...
    batch.files.clear();canvasLayers.clear();inputAliases.clear();inputTitles.clear();timelineTracks.clear();spaces.clear();activeSpaceId.clear();ensureSpaces();currentFile="aspectra://untitled";batch.files.append(currentFile);original=QImage(qMax(1,width),qMax(1,height),QImage::Format_ARGB32);original.fill(background.alpha()==0?Qt::transparent:background);media={};media.size=original.size();setDimensions(width,height);CanvasLayer layer;layer.source=currentFile;layer.name=artboard?"Artboard 1":"Canvas";layer.spaceId=activeSpaceId;layer.nativeSize=original.size();canvasLayers[currentFile]=layer;preview->clearVectorTraceFrame();preview->clearTextureFrame();preview->setLayerMask({});preview->setFrame(original);refresh();updateBatchLabel();if(exportButton)exportButton->setEnabled(true);QSettings settings;settings.setValue("newProjectResolution",resolution);settings.setValue("newProjectColorMode",colorMode);settings.setValue("newProjectProfile",profile);status->setText(QString("New %1 · %2 × %3 · %4 dpi · %5").arg(artboard?"artboard":"transparent canvas").arg(width).arg(height).arg(resolution).arg(colorMode));autosaveProject();
}
void MainWindow::autosaveProject(){
    if(!widthInput||!heightInput)return;
    ensureSpaces();QJsonObject document;document["version"]=3;document["recovery"]=true;
    document["savedAt"]=QDateTime::currentDateTimeUtc().toString(Qt::ISODate);
    document["canvas"]=QJsonObject{{"width",widthInput->value()},{"height",heightInput->value()},{"ratio",ratio?ratio->currentText():"Custom"}};document["activeSpaceId"]=activeSpaceId;QJsonArray spaceJson;for(const auto &space:spaces)spaceJson.append(QJsonObject{{"id",space.id},{"name",space.name},{"activeArtboard",space.activeArtboard},{"viewX",space.viewCenter.x()},{"viewY",space.viewCenter.y()},{"viewZoom",space.viewZoom}});document["spaces"]=spaceJson;
    QJsonArray layers;
    for(const QString &source:batch.files){
        const CanvasLayer layer=canvasLayers.value(source);
        QJsonObject item{{"source",source},{"name",layer.name},{"x",layer.position.x()},{"y",layer.position.y()},{"width",layer.nativeSize.width()},{"height",layer.nativeSize.height()},{"visible",layer.visible},{"opacity",layer.opacity},{"fill",layer.fill},{"blendMode",layer.blendMode},{"lockPosition",layer.lockPosition},{"spaceId",layer.spaceId}};
        if(!layer.mask.isNull())item["maskPngBase64"]=imageAsBase64(layer.mask);
...
void MainWindow::openProjectFile(const QString &path){
    ...
    QStringList sources;canvasLayers.clear();timelineTracks.clear();spaces.clear();activeSpaceId=root.value("activeSpaceId").toString();for(const auto &value:root.value("spaces").toArray()){const auto item=value.toObject();AspectraSpace space;space.id=item.value("id").toString();space.name=item.value("name").toString("Space");space.activeArtboard=item.value("activeArtboard").toString();space.viewCenter={item.value("viewX").toDouble(),item.value("viewY").toDouble()};space.viewZoom=item.value("viewZoom").toDouble(1);if(!space.id.isEmpty())spaces.append(space);}
    ...
    if(sources.isEmpty()){
        ...
        batch.files=sources;ensureSpaces();rebuildSpaceSelector();updateBatchLabel();const QStringList activeBoards=spaceArtboards();selectFile(activeBoards.isEmpty()?batch.files.first():activeBoards.first());
